...
class ExampleRobot:
    ...
    def __init__(self, name: str, server_url: str = "http://localhost:8000"):
        ...
        self.ws_url = f"ws://localhost:8000/ws/robot/{name}/"
    ...
    def start(self):
        ...
        # Iniciar loop principal usando asyncio
        asyncio.run(self.run_main_loop())
    
    async def run_main_loop(self):
        """Loop principal do robô (assíncrono)"""
        ...
        while self.is_running and not self.should_stop:
            try:
                ...
                # Enviar status a cada 30 segundos
                ...
                # Simular variações nos recursos do sistema
    ...
    def run_websocket(self):
        """Executar WebSocket em loop assíncrono"""
    ...
    def update_system_stats(self):
        """Atualizar estatísticas simuladas do sistema"""
        ...
        # Simular problemas de rede ocasionais
        if random.random() < 0.01:  # 1% de chance
            self.network_status = False
            self.send_log("WARNING", "Problema de conectividade de rede detectado")
        elif not self.network_status and random.random() < 0.3:  # 30% de chance de recuperar
            self.network_status = True
    ...
    def send_status_update(self):
        """Enviar atualização de status para a API"""
        try:
            data = {
                'robot_name': self.name,
                'status': self.status,
    ...
    def send_heartbeat(self):
        """Enviar heartbeat via WebSocket"""
        if self.websocket:
